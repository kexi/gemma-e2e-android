...
*なぜダッシュボードのシナリオビルダーもファイルを書くのか:* `POST /api/scenarios`
は YAML ローダーと同じ Zod スキーマでボディを検証し、`scenarios/<id>.yaml` を
書き出します。UI はディレクトリへの入口であって第 2 のストアではありません。
ブラウザで作ったシナリオも手書きのものとまったく同じようにレビュー・バージョン管理
され、CI で再実行されます。既に存在する id は上書きせず 409 を返します。git 管理下の
ファイルであり、黙って置き換えるとレビュー済みの成果を壊すためです。編集も第 2 の
入口ではなく同じ入口です。`PUT /api/scenarios/:id` は同じスキーマで検証し、同じ
ファイルを同じ体裁で書き換えるため、編集したシナリオは手で直したものと区別が
つきません。
